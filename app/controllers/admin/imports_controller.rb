class Admin::ImportsController < ApplicationController
  ...
  require 'excel_import_invitee_access'
  ...
  def redirect_hash(model_name, excel_errors= " ")
    ...
    hsh = {
      ...
      "invitee_access" => { "url" => admin_event_invitee_accesses_path(:event_id => params[:import][:importable_id].to_i), "template" => '/admin/imports/new' }               
    }
    hsh[model_name]
  end
  ...
  def decide_import_structure(file_url, type, importable_id,import_attribs, import_options)
    ...
    import_result = ExcelImportInviteeAccess.save(file_url, type, importable_id, import_attribs, 'add', import_options) if type == 'invitee_access'
    import_result
  end
end
