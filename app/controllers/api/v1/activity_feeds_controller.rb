  ...
  def index
    ...
    if event.present?
      if event.event_features.not_hidden_icon.pluck(:name).include? "conversations"
        @event_analytics = event.analytics.where(:viewable_type => ["Conversation","Notification", "InviteeNotification"], :action => ["comment", "conversation post", "like", "share", "notification"]).where("viewable_id is not null").order("created_at desc")
      else
        @event_analytics = event.analytics.where(:viewable_type => ["Notification", "InviteeNotification"]).where("viewable_id is not null").order("created_at desc")
      end
