  ...
  def index
    ...
    if event.present?
      ...
      member_last_name = member.last_name rescue ''
  		...
  		render :status => 200, :json => {:status => "Success", :member_first_name => member_first_name, :member_last_name => member_last_name, :chats => chats.as_json(:except => [:created_at, :updated_at], :methods => [:date_time_with_event_timezone])}
  	else
