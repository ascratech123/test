class Edm < ActiveRecord::Base
  ...
  def send_mail_to_invitees
    ...
    database_email_field = invitee_structure.email_field
    if edm.group_type == 'apply filter'
      registration = event.registrations.last
      registration_email_field = registration.email_field
      user_registrations = UserRegistration.where(:registration_id => registration.id)
      
      user_registration_emails = []
      if user_registrations.present?
        if edm.registered == 'yes'
          user_registration_emails = user_registrations.pluck("#{registration_email_field}")
        elsif edm.registered == 'no'
          user_registration_emails = user_registrations.pluck("#{registration_email_field}")
          if user_registration_emails.present?
      ...
      final_emails_arr = user_registration_emails & user_registration_approved_emails & confirm_emails & attended_emails
      ...
      final_emails_arr = InviteeDatum.where(:invitee_structure_id => invitee_structure.id).pluck("#{database_email_field}")
    end
