...
module ExcelImportPoll
#options => {:start_row => 'start_row_number'}
  def self.save(file_path, klass_name, event_id,attributes=[], operation='add', options={})
    attributes = Poll.column_names
    attributes -= ["id","created_at", "updated_at", "poll_start_date_time", "poll_end_date_time", "sequence", "poll_start_time", "poll_end_time"]
    objekts = self.prepare_objekts(file_path, klass_name, event_id, attributes, operation, options)
    errors = ExcelImportPoll.validate_objekts(objekts)
    if errors.blank?
      ExcelImportPoll.save_objekts(objekts)
