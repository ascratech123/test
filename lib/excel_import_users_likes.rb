...
#options => {:start_row => 'start_row_number'}
  def self.save(file_path, klass_name, event_id,attributes=[], operation='add', options={})
    ...
    objekts = ExcelImportUserLike.prepare_objekts(file_path, klass_name, event_id, attributes, operation, options)
    errors = ExcelImportUserLike.validate_objekts(objekts)
    if errors.blank?
      ExcelImportUserLike.save_objekts(objekts)
  ...
  def self.prepare_objekts(file_path, klass_name, event_id, attributes, operation, options)
    ...
    start_row.upto(workbook.last_row) do |line|
      ...
      like = Like.find_or_initialize_by(:user_id => user_id, :likable_id => conversation_id, :likable_type => 'Conversation')
      objekts << like
    end
    objekts.compact
  end
