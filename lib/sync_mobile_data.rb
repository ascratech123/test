module SyncMobileData
  ...
  def self.sync_records(start_event_date, end_event_date,mobile_application_id,current_user,submitted_app, event_ids = nil, all_mobile_event_ids = nil)
    ...
    # data["all_events"] = Event.where(:id => all_mobile_event_ids).as_json(:only => [:id, :event_name, :cities, :venues, :logo_updated_at, :status, :inside_logo_updated_at, :theme_id, :login_at, :event_category, :marketing_app, :start_event_time, :end_event_time], :methods => [:logo_url,:inside_logo_url]) if all_mobile_event_ids.present?
    return data
  end  
