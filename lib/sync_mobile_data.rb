  ...
  def self.sync_records(start_event_date, end_event_date,mobile_application_id,current_user,submitted_app)
    ...
    model_name.each do |model|
      ...
      case model
        ...
        when "Agenda"
          data[:"#{name_table(model)}"] = info.as_json(:except => [:sequence], :methods =>[:agenda_track_name, :track_sequence, :formatted_start_date_detail, :formatted_time, :formatted_start_date_listing])
        else
          data[:"#{name_table(model)}"] = info.as_json() rescue []
      end  
    end
