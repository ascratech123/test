    ...
    model_name.each do |model|
      ...
      case model
        when 'Conversation'
          ...
          data[:"#{name_table(model)}"] = info.includes(:event).as_json(:except => [:image_file_name, :image_content_type, :image_file_size, :image_updated_at], :methods => [:image_url,:company_name,:like_count,:user_name,:comment_count])
        when 'EmergencyExit'
          data[:"#{name_table(model)}"] = info.includes(:event).as_json(:except => [:icon_file_name,:icon_content_type,:icon_file_size,:icon_updated_at,:emergency_exit_file_name, :emergency_exit_content_type, :emergency_exit_size, :emergency_exit_updated_at, :uber_link], :methods => [:emergency_exit_url,:icon_url, :attachment_type])
        when 'Event'
        ...
        when 'Speaker'
          data[:"#{name_table(model)}"] = info.includes(:event).as_json(:except => [:profile_pic_file_name, :profile_pic_content_type, :profile_pic_file_size, :profile_pic_updated_at], :methods => [:profile_pic_url, :is_rated]) rescue [] 
        when 'Image'
        ...
        when 'Winner'
          award_ids = Award.where(:event_id => event_ids).includes(:winners) rescue nil
        ...
        when 'Comment'
          conversation_ids = Conversation.where(:event_id => event_ids).includes(:comments) rescue nil
        ...
        when 'Exhibitor'
          data[:"#{name_table(model)}"] = info.includes(:event).as_json(:except => [:updated_at, :created_at, :image_file_name, :image_content_type, :image_file_size, :image_updated_at], :methods => [:image_url]) rescue []  
        when 'Notification'
        ...
        when 'Like'
          if current_user.present?
            conversation_ids = Conversation.where(:event_id => event_ids).includes(:likes) rescue nil
            info = Like.where(:likable_id => conversation_ids, likable_type: "Conversation", :updated_at => start_event_date..end_event_date) rescue []
            data[:"#{name_table(model)}"] = info.as_json() rescue []
          end
        when 'UserPoll'
          if current_user.present?
            poll_ids = Poll.where(:event_id => event_ids).includes(:user_polls) rescue nil
            info = UserPoll.where(:poll_id => poll_ids, :updated_at => start_event_date..end_event_date) rescue []
            data[:"#{name_table(model)}"] = info.as_json() rescue []
          end
        when 'UserQuiz'
          if current_user.present?
            quiz_ids = Quiz.where(:event_id => event_ids).includes(:user_quizzes) rescue nil
        ...
        when 'UserFeedback'  
          if current_user.present?
            feedback_ids = Feedback.where(:event_id => event_ids).includes(:user_feedbacks) rescue nil
        ...
        when "Agenda"
          data[:"#{name_table(model)}"] = info.includes(:event).as_json(:except => [:sequence], :methods =>[:agenda_track_name, :track_sequence])
        else
          data[:"#{name_table(model)}"] = info.as_json() rescue []
      end  
    end
